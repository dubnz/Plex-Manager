...
from app.services.sync import _request_indexes_from_sources, _watch_stats_from_history


def test_plex_parser_sums_media_part_sizes() -> None:
    node = ElementTree.fromstring(
        """
        <Video ratingKey="42" title="Example" type="movie" year="2024" addedAt="1710000000">
          <Media>
            <Part size="1024" />
            <Part size="2048" />
          </Media>
...
def test_request_indexes_preserve_current_source_then_fill_legacy() -> None:
    ...
    assert indexes.find(rating_key="10", manager_kind="radarr", manager_id=5).requested_by == "Current User"
    assert indexes.find(rating_key="20", manager_kind="sonarr", manager_id=9).requested_by == "Show Requester"
